...
export class GUI {
  ...
  addMap(mapPlane) {
    ...
    mapFolder.addBinding(CONFIG.mapPlane, 'correction', {
      x: { min: -500, max: 500 },
      y: { min: -500, max: 500 },
      z: { min: -500, max: 500 }
    }).on('change', () => {
    ...
    mapFolder.addBinding(CONFIG.mapPlane, 'planePositionZ', { min: -1000, max: 0 }).on('change', () => {
      if (mapPlane) mapPlane.update();
    });
    mapFolder.addBinding(CONFIG.mapPlane, 'planeRotationX', { min: -Math.PI, max: Math.PI }).on('change', () => {
      if (mapPlane) mapPlane.update();
    });
    mapFolder.addBinding(CONFIG.mapPlane, 'backgroundZ', { min: -100, max: 10 }).on('change', () => {
      if (mapPlane) mapPlane.update();
    });
  }
  ...
  addLights(ambientLight, directionalLight) {
    ...
    lightsFolder.addBinding(CONFIG.lights, 'ambientIntensity', { min: 0, max: 2 }).on('change', (ev) => {
      if (ambientLight) ambientLight.intensity = ev.value;
    });
    lightsFolder.addBinding(CONFIG.lights, 'directionalIntensity', { min: 0, max: 5 }).on('change', (ev) => {
      if (directionalLight) directionalLight.intensity = ev.value;
    });
    lightsFolder.addBinding(CONFIG.lights, 'directionalPosition', {
      x: { min: -50, max: 50 },
      y: { min: -50, max: 50 },
      z: { min: -50, max: 50 },
    }).on('change', (ev) => {
  ...
  addCamera(camera) {
    ...
    cameraFolder.addBinding(CONFIG.camera, 'fov', { min: 10, max: 120 }).on('change', (ev) => {
      if (camera) {
        camera.fov = ev.value;
  ...
  addExport() {
    this.pane.addButton({ title: 'Export Config to Console' }).on('click', () => {
